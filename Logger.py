...
class Logger:
    ...
    def start(self) -> None:
        ...
        # Nagłówki zgodne z nowym formatem (6 kolumn)
        headers = ['timestamp', 'sensor_id', 'value', 'unit', 'level', 'message']
        ...
        # Zawsze dopisz nagłówek jeśli plik jest nowy lub pusty
    ...
    def log_reading(self, sensor_id: str, timestamp: datetime, value: float, unit: str, additional_info: dict = None) -> None:
        """
        Dodaje wpis do bufora i ewentualnie wykonuje rotację pliku.
        """
        row = [timestamp.isoformat(), sensor_id, value, unit, "", ""]
        if additional_info:
            row[4] = additional_info.get("level", "")
            row[5] = additional_info.get("message", "")
    ...
    def info(self, message: str) -> None:
        """Rejestruje informacyjny wpis w logu."""
        self.log_reading(
            sensor_id="SYSTEM",
            timestamp=datetime.now(),
            value=0.0,
            unit="LOG",
            additional_info={"level": "INFO", "message": message}
        )

    def error(self, message: str) -> None:
        """Rejestruje błąd w logu."""
        self.log_reading(
            sensor_id="SYSTEM",
            timestamp=datetime.now(),
            value=0.0,
            unit="LOG",
            additional_info={"level": "ERROR", "message": message}
        )